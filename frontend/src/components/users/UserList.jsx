...
export default function UserList({ darkMode }) {
  ...
  // Function: Opens form for create/edit
  ...
  // Memo: Filter for search efficiency
    ...
    <div className={`relative m-5 p-3 `}>
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-3">
        <div className="flex flex-row justify-between items-center gap-3 w-full">
          ...
          <Button type="button" isDarkMode={darkMode} label="Add User" icon={<Plus className="w-4 h-4" />} 
            onClick={() => openForm()} className="bg-indigo-600 hover:bg-indigo-700 text-white" />
        </div>
      </div>

      {/* Table */}
      {/* Overflow: Max-h for scrollable table; hidden scrollbar for clean look */}
      <div className={`max-h-[640px] overflow-auto [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden rounded-lg"  ${darkMode.componentsBgMode}`}>
        <table className="w-full border-collapse">
          <thead className={`sticky top-0 z-10 text-left text-sm uppercase text-white ${darkMode.darkMode ? 'bg-gray-800 text-gray-300' : 'bg-gray-500 text-gray-700'}`}>
            <tr>
            ...
            {filteredUsers.length === 0 ? (
              ...
              filteredUsers.map((u) => (
                <tr key={u.id} className={`transition-colors text-start`}>
                  ...
                  {/* Icon: UserCircle2 for visual; fallback '—' if no name */}
                  ...
                  {/* Actions: Spinner on delete for feedback */}
      ...
      <AnimatePresence>
        {showForm && (
          <>
          {/* Backdrop: Click to close; motion for fade */}
      ...
      {deleteUserId && (
        // Assumes: Title "User" for generic delete
